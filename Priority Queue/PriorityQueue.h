...
private:
    ...
    void downheap(size_type index) {
        ...
        size_type swapIndex = left_child(index);

        if(right_child(index) < c.size() && comp(c[swapIndex], c[right_child(index)])) //checking if left of tree is smaller than right???
        {
            swapIndex = right_child(index);
        }

        if(comp(c[swapIndex], c[index]))
        {
            return;
        }

        std::swap(c[swapIndex], c[index]);
        downheap(swapIndex);
